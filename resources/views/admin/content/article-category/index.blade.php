...
@section('content')
    ...
    <nav aria-label="breadcrumb">
        <ol class="breadcrumb">
            ...
            <li class="breadcrumb-item font-size-12 active" aria-current="page">دسته بندی مقالات</li>
        </ol>
    </nav>
            ...
            <section class="main-body-container">
                ...
                <section class="d-flex justify-content-between align-items-center mt-4 mb-3 border-bottom pb-2">
                    @if(auth()->user()->hasPermissionTo('create-article'))
                        <a href="{{ route('admin.content.article-category.create') }}" class="btn btn-info btn-sm">ایجاد
                            دسته بندی</a>
                    @endif
                        ...
                        @foreach ($categories as $key=>$category)
                            ...
                            <tr>
                                    ...
                                    <label>
                                        @if(auth()->user()->hasPermissionTo('edit-article'))
                                            <input id="{{ $category->id }}" onchange="changeStatus({{ $category->id }})"
                                                   data-url="{{ route('admin.content.article-category.status', $category->id) }}"
                                ...
                                <td class="width-16-rem text-left">
                                    ...
                                    @if(auth()->user()->hasPermissionTo('edit-article'))
                                        <a href="{{ route('admin.content.article-category.edit', $category->id) }}"
                                    ...
                                    @if(auth()->user()->hasPermissionTo('delete-article'))
                                        <form class="d-inline"
                                              action="{{ route('admin.content.article-category.destroy', $category->id) }}"
                                              method="post">
                                            @csrf
                                            {{ method_field('delete') }}
